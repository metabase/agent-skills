...
I need to embed three dashboards in my React app: a sales overview, a user growth chart, and a top products report. The sales one should be interactive so users can filter it. My Metabase is at http://localhost:13000. I'm using JWT SSO auth — the signing endpoint is already set up at `/api/metabase/auth`.
...
- The agent uses JWT SSO auth configuration, not API key auth
