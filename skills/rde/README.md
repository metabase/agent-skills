...
A skill for an agent working as a data engineer at a company whose stack is unknown, with Metabase at the center: explore and profile raw data, build clean tables as Metabase transforms, define the semantic layer (models, measures, segments, metrics, metadata), build dashboards, answer questions, and reconcile a number against a reference. `SKILL.md` routes; each job runs one playbook, resumable from `./.scratch/STATE.md`.
...
- `playbooks/build-clean-tables.md`: build the models as transforms with a quality gate per model, hide the plumbing, schedule the chain, leave a standing check.
